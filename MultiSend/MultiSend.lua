...
UMMMailComposerMultiSend = {
  ...
  AutoBuildMail = function(self)
    ...
    UMMSetGlobalFailTimeout("UMMMailComposerMultiSend");
  end;
...
function UMMComposeMailTemplateMultiSend_OnLoad(this)
  ...
  this.ClearMail = function(self, full)
    ...
    UMMMailComposerMultiSend:Clear();
  end;
  ...
  this.Send = function(self)
    ...
    UMMMailComposerMultiSend:Clear();
    UMMMailComposerMultiSend:SetComposer(self:GetName());
    UMMMailComposerMultiSend:Recipient(recipient);
    UMMMailComposerMultiSend:Subject(subject);
    UMMMailComposerMultiSend:Body(body);
    if (getglobal(self:GetName().."FooterOptionCOD"):IsChecked()) then
      ...
      if (money ~= nil and money ~= "") then
        ...
        cash = cash - 1;
        if (cash > 0) then
          UMMMailComposerMultiSend:DefinedCODAmount(cash);
        end
      end
    else
      local money = getglobal(self:GetName().."FooterMoney"):GetText();
      if (money ~= nil and money ~= "") then
        ...
        cash = cash - 1;
        if (cash > 0) then
          UMMMailComposerMultiSend:AttachedMoney(cash);
        end
      end
    end
    if (self.AttachBagSlotIndex > 0) then
      UMMMailComposerMultiSend:AttachedItemIndex(self.AttachBagSlotIndex);
    end
    local result = UMMMailComposerMultiSend:Send();
    if (result == 0) then
...
function UMMComposeMailTemplateMultiSend_OnShow(this)
  this:ClearMail(true);
  this:CheckSendStatus();
  getglobal(this:GetName().."HeaderAuthor"):SetFocus();
end

function UMMComposeMailTemplateMultiSend_OnHide(this)
  this:ClearMail();
  this:EnableComposer();
  this:ClearAllEditFocus();
end

function UMMComposeMailTemplateMultiSendButton_OnClick(this, action)
